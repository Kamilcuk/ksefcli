...
public class WystawPodobnaFaktureCommand : IGlobalCommand {
    ...
    public override async Task<int> ExecuteAsync(CancellationToken cancellationToken) {
        ...
        XElement? naglowek = doc.Root?.Element(ns + "Naglowek");
        if (naglowek != null) {
            XElement? dataWytworzenia = naglowek.Element(ns + "DataWytworzeniaFa");
        ...
        if (fa != null) {
            XElement? p1 = fa.Element(ns + "P_1");
            if (p1 != null) {
                p1.Value = p1Date;
            }

            XElement? p6 = fa.Element(ns + "P_6");
            if (p6 != null) {
                p6.Value = p6Date;
            }

            // Update P_2 (Invoice number) based on new P_1 if it follows the pattern FV/yyyyMMdd/01
            XElement? p2 = fa.Element(ns + "P_2");
