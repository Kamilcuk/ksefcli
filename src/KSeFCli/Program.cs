    ...
    return AsyncRunner.Run(async () => {
      ...
      try {
        ...
        InvoiceQueryFilters invoiceQueryFilters = new InvoiceQueryFilters {
          ...
          DateRange = new DateRange { From = opts.From, To = opts.To,
                                      DateType = dateType }
        };
  ...
  static int RunExportInvoices(ExportInvoicesOptions opts) {
    return AsyncRunner.Run(async () => {
      ...
      try {
        ...
        ICryptographyService cryptographyService =
            new CryptographyService(certificate);
        ...
        InvoiceExportRequest invoiceExportRequest = new InvoiceExportRequest {
          Encryption = encryptionData.EncryptionInfo, Filters = queryFilters
        };
    ...
    static int RunGetExportStatus(GetExportStatusOptions opts) {
      return AsyncRunner.Run(async () => {
        IKSeFClient ksefClient = CreateKSeFClient(opts.BaseUrl);
        try {
          InvoiceExportStatusResponse exportStatus =
              await ksefClient.GetInvoiceExportStatusAsync(opts.ReferenceNumber,
                                                           opts.Token);

          Console.WriteLine(JsonSerializer.Serialize(
              new { Status = "Success", ExportStatus = exportStatus }));
        } catch (Exception ex) {
          Console.Error.WriteLine(JsonSerializer.Serialize(
              new { Status = "Error", Message = ex.Message }));
          return 1;
