...
public class TokenAuthCommand : GlobalCommand
{
    public override async Task<int> ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        string tokenWithTimestamp = $"{Token}|{timestampMs}";
        byte[] tokenBytes = Encoding.UTF8.GetBytes(tokenWithTimestamp);
        byte[] encrypted = cryptoService.EncryptKsefTokenWithRSAUsingPublicKey(tokenBytes);
        ...
        do
        {
            status = await ksefClient.GetAuthStatusAsync(signature.ReferenceNumber, signature.AuthenticationToken.Token, cancellationToken).ConfigureAwait(false);
